...
if(length(which(depth >= chl_d)) > 30){
  ...
  if( all( ((deep - mean(deep, na.rm = T))/diff(range(fluor, na.rm = T) )) < 0.05, na.rm = T)){
    ...
    if(length(fluor_mod_down) == 1){if(is.na(fluor_mod_down)){fluor_mod_down = list("fluor.out" = rep(mean(deep, na.rm = T),length(deep)),"bp" = NULL)}
    }}
...
fluor_adj[which(fluor_adj > mean(fluor_adj[depth > chl_d], na.rm = T)+6*sd(fluor_adj[depth > chl_d], na.rm = T) & depth > chl_d)] = NA
...
abstract_bbp = function(depth,bbp,fluor){
  d = depth[is.finite(bbp) & depth < 100]
